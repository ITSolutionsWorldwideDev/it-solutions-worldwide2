...
export default function BlogCard({ post }: { post: BlogEntry }) {
  const truncateText = (text: string, wordLimit: number) => {
    const words = text.split(' ');
    return words.length > wordLimit
      ? words.slice(0, wordLimit).join(' ') + '...'
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-6 flex flex-col lg:flex-row-reverse items-center border-gray-200 border">
      <div className="w-full lg:w-2/4 mb-4 lg:mb-0">
        <img
          src={getFeaturedImage()}
          alt={post.content.title}
      ...
      <div className="w-full lg:w-2/4 p-4">
        <h3 className="text-lg font-bold mt-2">{post.content.title}</h3>
